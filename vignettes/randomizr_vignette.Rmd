...
# Transform so each row is a subject, rather than a type of subject.
...
The advantage is real, and it is visible when the sample is small. Repeating the same exercise with 10 units rather than 592:
...
small_estimates <-
  map(1:sims, function(i) {
    ...
    tibble(
      # simple_ra() can put every unit in the same condition, which leaves the
      # difference-in-means undefined. Those draws are exactly the cost of the design.
      simple = if (n_distinct(small$Z_simple) == 2) {
        mean(small$Y_simple[small$Z_simple == 1]) - mean(small$Y_simple[small$Z_simple == 0])
      } else {
        NA_real_
      },
      complete = mean(small$Y_complete[small$Z_complete == 1]) -
        mean(small$Y_complete[small$Z_complete == 0])
    )
  }) |>
  list_rbind()
...
At `N = 10` the gap is large enough to see, and simple random assignment additionally produces draws in which every unit landed in the same condition and no treatment effect can be estimated at all. That is the practical case for `complete_ra()`: not that it buys a lot of precision in a large sample, but that it removes an avoidable source of variability, guarantees the number of units you can afford to treat, and never hands you a useless assignment.
...
tibble(Hair = hec$Hair, Z) |> count(Hair, Z) |> pivot_wider(names_from = Z, values_from = n)
```
```{r echo=FALSE}
knitr::kable(tibble(Hair = hec$Hair, Z) |> count(Hair, Z) |> pivot_wider(names_from = Z, values_from = n))
```
...
```
```{r echo=FALSE}
knitr::kable(tibble(Hair = hec$Hair,
                    prob = round(declaration$probabilities_matrix[, 2], 3)) |>
               count(Hair, prob) |>
               pivot_wider(names_from = prob, values_from = n))
```
...
```{r echo=TRUE, results="hide"}
# A standalone vector, not a column: `blocks` is built from the cluster sizes
# further down, and two different things should not share one column name.
blocks <- paste(hec$Hair, hec$Eye, hec$Sex, sep = "_")
Z <- block_ra(blocks = blocks)
tibble(blocks, Z) |> count(blocks, Z) |>
  pivot_wider(names_from = Z, values_from = n) |> head()
```
```{r echo=FALSE}
knitr::kable(tibble(blocks, Z) |> count(blocks, Z) |>
               pivot_wider(names_from = Z, values_from = n) |> head())
```
...
hec <- hec |> mutate(block_id = createBlockIDs(out, df_forBT, id.var = "id_var"))
...
```{r echo=TRUE, results="hide"}
hec <- hec |> mutate(clusters = paste(Hair, Eye, Sex, sep = "_"))
clusters <- hec$clusters

Z_clust <- cluster_ra(clusters = clusters)

tibble(clusters, Z_clust) |> count(clusters, Z_clust) |>
  pivot_wider(names_from = Z_clust, values_from = n) |> head()
```
```{r echo=FALSE}
knitr::kable(tibble(clusters, Z_clust) |> count(clusters, Z_clust) |>
               pivot_wider(names_from = Z_clust, values_from = n) |> head())
```
...
hec <- left_join(hec, cluster_level_df, by = "clusters")
...
tibble(clusters, Z) |> count(clusters, Z) |>
  pivot_wider(names_from = Z, values_from = n) |> head()
tibble(blocks, Z) |> count(blocks, Z) |>
  pivot_wider(names_from = Z, values_from = n) |> head()
```
```{r echo=FALSE}
knitr::kable(tibble(clusters, Z) |> count(clusters, Z) |>
               pivot_wider(names_from = Z, values_from = n) |> head())
knitr::kable(tibble(blocks, Z) |> count(blocks, Z) |>
               pivot_wider(names_from = Z, values_from = n) |> head())
```
...
```
```{r echo=FALSE}
knitr::kable(tibble(Hair = hec$Hair, Z) |>
               count(Hair, Z) |>
               pivot_wider(names_from = Z, values_from = n))
```
...
```
```{r echo=FALSE}
knitr::kable(tibble(cond_prob = round(cond_prob, 2), Z) |>
               count(cond_prob, Z) |>
               pivot_wider(names_from = Z, values_from = n))
```
