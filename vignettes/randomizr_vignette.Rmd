...
`balanced_ra()` is experimental; its interface may change.

Six units, two blocks of three, half assigned to treatment. `complete_ra(N = 6, m = 3)` treats three units every time, but a block can receive zero or three. That is a target complete assignment cannot hit: overall half, and one or two treated in each block.
...
Every draw treats three units overall and one or two in each block. Declare the design with `declare_ra(..., ra_type = "balanced")`. The [introduction article](https://declaredesign.org/r/randomizr/articles/balanced_ra.html) has the details.
