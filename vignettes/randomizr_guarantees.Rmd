...
It says nothing about designs it does not draw. `balanced_ra()` with three or more arms and `blocks` is the case worth naming, because there the within-block counts stay tight and the overall count does not: it is the sum of independent block landings and can move by more than one. That is a documented limit of the implementation rather than an oversight, and the "Introduction to balanced_ra" article gives the reason.
