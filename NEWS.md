...
`declare_ra()` refuses a single number in a per-unit slot and names the argument that takes one. `prob` and `m` are the scalar slots and `prob_unit` and `m_unit` the per-unit ones, but only the complete, blocked, clustered and simple paths enforced that; the balanced path recycled a scalar `prob_unit` instead, so one argument meant two different things depending on the design. `declare_ra(prob_unit = 0.5, ra_type = "balanced")` now says to use `prob`, which has always worked on that path, and `m_unit` likewise points at `m`. N = 1 is exempt, since there one value is one value per unit. `balanced_ra()` called directly is unchanged and goes on recycling: it has no `prob` argument, so a scalar `prob_unit` is how a shared probability is written there, and `balanced_ra(4)` still means what it says.
