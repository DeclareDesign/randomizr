...
`balanced_ra()` gains `formula` and `data` for cube-on-X: `formula = ~ x + B` balances the treated totals of those columns (the intercept is the count constraint) without cutting covariates into blocks. `blocks` remains the partitioned count path. Passing both errors. `formula` with `prob_unit_each` errors (two-arm only). `declare_ra(..., formula = )` selects the balanced path and passes the formula through. The count-tight `blocks` C++ path is unchanged when `formula` is `NULL`. See `vignette("balanced_ra_covariates")`.
